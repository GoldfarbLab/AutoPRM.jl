function SearchPRM(params_path::String)
    params = JSON.parse(read(params_path, String));
    MS_DATA_DIR = params["ms_data_dir"];
    TRANSITION_LIST_PATH = params["transition_list_path"];
    ...
    for (ms_file_idx, MS_TABLE_PATH) in ProgressBar(enumerate(MS_TABLE_PATHS))
            ...
            println("length(fragment_matches) ", length(fragment_matches))
        ...
        Threads.@threads for (ms_file_idx, MS_TABLE_PATH) in ProgressBar(collect(enumerate(MS_TABLE_PATHS)))
            ...
            getMS1PeakHeights!(ms1_peak_heights[ms_file_idx],
                ptable,
                                ...
                                params[:left_precursor_tolerance],
                                UInt32(ms_file_idx))
                                println("ms_file_idx $ms_file_idx finished")
        end
    end
